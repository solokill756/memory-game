...
class MemoryGame {
  ...
  changeLevel(level: 'easy' | 'medium' | 'hard'): void {
    switch (level) {
      case 'easy':
        this.cardCount = 8;
        this.updateGameBoardLayout(this.cardCount);
  ...
  updateScore(): void {
    ...
    document.getElementById(
      'best-score'
    )!.textContent = `Best Score: ${this.bestScore}`;
  }
  ...
  updateTimer(): void {
    ...
    if (timerEl) timerEl.textContent = `Time: ${this.timer}s`;
  }
  ...
  updateGameBoardLayout(cardCount: 8 | 18 | 32): void {
